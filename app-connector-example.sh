...
while [[ -z $COMMERCE_WEBSERVICES_ID || $COMMERCE_WEBSERVICES_ID=="null"  ]]; do echo "registering commerce webservices"; COMMERCE_WEBSERVICES_ID=$(curl -sk 'https://commerce.local.kyma.dev/local/apis/Commerce%20Webservices/register' -H 'content-type: application/json' -H 'origin: https://commerce.local.kyma.dev' -d '{}' --compressed | jq -r '.id'); echo "COMMERCE_WEBSERVICES_ID=$COMMERCE_WEBSERVICES_ID"; sleep 2; done

COMMERCE_EVENTS_ID=""
while [[ -z $COMMERCE_EVENTS_ID || $COMMERCE_EVENTS_ID=="null"]]; do echo "registering commerce events"; COMMERCE_EVENTS_ID=$(curl -sk 'https://commerce.local.kyma.dev/local/apis/Events/register' -H 'content-type: application/json' -H 'origin: https://commerce.local.kyma.dev' -d '{}' --compressed | jq -r '.id'); echo "COMMERCE_EVENTS_ID=$COMMERCE_EVENTS_ID" sleep 2; done
